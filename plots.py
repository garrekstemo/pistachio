...
def set_units(unit_data):
	...
	energy_to_ev = [joule_to_ev(en) for en in energy]
	wavenumber_to_wavelength = [wavenum_to_wavelen(d) for d in unit_data]
	
	if args.ev_units:
		new_units = energy_to_ev
	elif args.wavenumber_units:
		new_units = unit_data
	elif args.wavelength_units:
...
def plot_dispersion(file_prefix, dispersion_file):
	...
	up = set_units(up)
	lp = set_units(lp)
	cavity = set_units(cavity)
	vibration = set_units(vibration)
...
# 	ax.plot(angles, cavity, linestyle='dashed', color=color1)
...
if __name__ == "__main__":
	...
	ev_help = "Sets units to eV for dispersion curve."
	wn_help = "Sets units to cm^-1 for dispersion curve."
	wl_help = "Sets units to micrometers for dispersion curve."
	...
	parser.add_argument('-EV', '--ev_units', action='store_true', help=ev_help)
	parser.add_argument('-WN', '--wavenumber_units', action='store_true', help=wn_help)
	parser.add_argument('-WL', '--wavelength_units', action='store_true', help=wl_help)
